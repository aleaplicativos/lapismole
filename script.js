...
console.clear();
class Lerper {
    constructor(container, pointsCount, offset, speed) {
        this.ready = false;
        this.points = [];
...
class Drawer {
    ...
    onMove(point) {
        if (this.drawing || CodePen.isThumbnail) {
            this.pathPoints.push(...[point.x, point.y]);
            this.path.setAttribute('d', `M ${this.pathPoints[0]} ${this.pathPoints[1]} L ${this.pathPoints.join(' ')}`);
        }
    }
    onUp() {
        this.drawing = false;
    ...
    newPath() {
        this.pathPoints = [];
        this.path = document.createElementNS("http://www.w3.org/2000/svg", 'path');
        this.path.setAttribute('class', 'pencil-line');
...
class App {
    constructor(container) {
        ...
        this.follower = null;
        this.drawer = null;
        this.followTranslator = null;
        this.pencilElements = [];
        this.pencilReferences = [];
        this.pencilTranslated = [];
        this.container = container;
        this.svg = document.getElementById('stage');
        this.input = CodePen.input();
    ...
    start() {
        ...
        if (guideContainer && translatedContainer) {
            this.follower = new Lerper(guideContainer, 25, { x: 3, y: -7 }, 1.03);
            this.drawer = new Drawer(drawContainer, this.svg);
            this.followTranslator = new POP.TranslationPath(this.follower.path, 30, 30, 0.1);
            this.input.onDown(point => {
                this.drawer.onDown(point);
            });
            this.input.onMove(point => {
                this.follower.onMove(point);
                this.drawer.onMove(point);
            });
            this.input.onUp(point => {
                this.drawer.onUp();
            });
            for (let i = 0; i < pencilElements.length; i++) {
                let ref = new POP.ReferencePath(pencilElements[i], 200, { x: 0, y: 0 }, { x: 122, y: 507 });
                this.pencilReferences.push(ref);
    ...
    tick() {
        if (this.followTranslator) {
            ...
            for (let i = 0; i < this.pencilReferences.length; i++) {
                let ref = this.pencilReferences[i];
